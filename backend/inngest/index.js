...
export const inngest = new Inngest({ id: "employee-portal" });
...
const autoCheckOut = inngest.createFunction(
    ...
    async ({ event, step }) => {
        ...
        if (!attendance?.checkOut) {
            ...
            await sendEmail({
                to: employee.email,
                subject: "Reminder: Attendance check-out Reminder",
                body: `
        ...
        return { message: "Attendance already checked out" }
    },
);


// Send Email to admin, if admin does not take action to leave application within 24 hours
    ...
    async ({ event, step }) => {
        ...
        if (leaveApplication?.status === 'PENDING') {
            ...
            await sendEmail({
                ...
                subject: "Reminder: Leave Application Pending",
                body:  `
            <div style="max-width: 600px;">
                <h2>Hi Admin, 👋</h2>
...
const attendanceReminderCron = inngest.createFunction(
    ...
    async({step}) => {
        ...
        if(absentEmployees.length > 0){
            await step.run("send-absent-reminders", async() => {
